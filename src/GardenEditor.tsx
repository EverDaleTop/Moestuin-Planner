...
import type { Crop, EditorTool, ElementType, Garden, GardenElement } from "./types";
...
const IconSelect = () =>
  icon(<path d="M5 3l14 8-6 1.5L10 18l-2-8z" />);
const IconMove = () =>
  icon(
    <g>
...
const IconFrame = () => icon(<rect x="4" y="4" width="16" height="16" rx="2" />);
const IconBed = () =>
  icon(<g><path d="M4 18V9h16v9" /><path d="M4 13h16" /><circle cx="7" cy="11" r="1.2" /><circle cx="10" cy="11" r="1.2" /></g>);
const IconPath = () =>
  icon(<g><circle cx="19" cy="5" r="1.4" /><circle cx="12" cy="12" r="1.4" /><circle cx="5" cy="19" r="1.4" /><path d="M18 5l-12 12" /></g>);
const IconTrash = () =>
  icon(<g><path d="M4 7h16" /><path d="M9 7V4h6v3" /><path d="M6 7l1 13h10l1-13" /><path d="M10 11v5M14 11v5" /></g>);
const IconCopy = () =>
  icon(<g><rect x="8" y="8" width="11" height="11" rx="2" /><path d="M16 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h3" /></g>);
const IconUndo = () => icon(<g><path d="M9 7L4 12l5 5" /><path d="M4 12h11a5 5 0 0 1 0 10" /></g>);
const IconRedo = () => icon(<g><path d="M15 7l5 5-5 5" /><path d="M20 12H9a5 5 0 0 0 0 10" /></g>);
...
interface Props {
  ...
  onBack: () => void;
  onAddFrame: (type: "bed" | "path", x: number, y: number, widthM: number, heightM: number) => string;
  ...
  onApplyElements: (updates: { id: string; x?: number; y?: number; widthM?: number; heightM?: number }[]) => void;
...
export function GardenEditor(props: Props) {
  ...
  // Delete / Backspace removes the selected elements (except while typing).
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (selectedIds.length === 0 || isEditable(e.target)) return;
  ...
  // Ctrl+D duplicates the selected element. preventDefault stops the browser's
  // "add bookmark" dialog. Use a ref so the listener always reads the latest
  // handler/data (otherwise a duplicate right after a resize would copy the
  ...
  return (
    <div className="ge-wrap">
      <header className="ge-header">
        <button className="btn-ghost" onClick={onBack} title="Terug naar tuinen">
          ← Tuinen
        </button>
        <input
          className="ge-title"
      ...
      <div className="ge-body">
        <div className="ge-canvas">
          ...
          />
          <div className="ge-bottom-toolbar" role="toolbar" aria-label="Gereedschap">
            <div className="ge-tools" role="group" aria-label="Gereedschap">
              <button
            ...
            <div className="ge-tools" role="group" aria-label="Te tekenen element">
              <button
            ...
            <div className="ge-tools" role="group" aria-label="Verwijderen">
              <button
            ...
            <div className="ge-tools" role="group" aria-label="Ongedaan maken">
              <button
                onClick={onUndo}
        ...
        <Inspector
          ...
          onRemoveCrop={(iId) => selected && onRemoveCrop(selected.id, iId)}
